...
def generate_user_report(highlight_changes : bool, save_to_history : bool, only_changes : bool, user : str):
    service_manager = ServiceManager()
    html_gen = HtmlGenerator()
    pdf_gen = PdfGenerator()

    old_services_df = service_manager.try_get_last_services(user)
    services_df = service_manager.get_services_for_user(user)
    if save_to_history:
        service_manager.save_services(services_df, user)
    html_tables = html_gen.generate_for_all_services(services_df, old_services_df, highlight_changes, only_changes)
